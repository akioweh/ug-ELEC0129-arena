...
- Invoke via the `Agent` tool with `subagent_type: "transcriber"`.
...
- For multiple independent transcription jobs (e.g. one per week of a lecture series), dispatch multiple `Agent` calls in a single message — they run concurrently.
